...
add_action ( 'add_meta_boxes_movie_listing', 'ml_add_fields_metabox' );
  ...
  ?>
    <div class="wrap movie-listing-form">
        <div class="form-group">
            ...
            <input class="full" type="text" name="movie_id" id="movie_id" value="<?php if( ! empty( $ml_stored_meta['movie_id'] ) ) echo esc_attr( $ml_stored_meta['movie_id'][0] );?>"/>
        </div>

        <div class="form-group">
            <label for="movie-id"><?php esc_html_e( 'Movie Listing ID','ml-domain' );?></label>
        ...
        <?php
            if ( get_settings( 'ml_setting_show_editor' ) ):
        ?>
        <div class="form-group">
        ...
        <?php else : ?>
            <div class="form-group">
                <label for="details"><?php esc_html_e( 'Details','ml-domain' );?></label>
                <textarea class="full" name="details" id="details" /><?php if( ! empty( $ml_stored_meta['details'] ) ) echo esc_html_e( $ml_stored_meta['details'][0] );?></textarea>
            </div>
        <?php endif; ?>       
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input type="text" class="full" name="director" id="director" value="<?php if( ! empty( $ml_stored_meta['director'] ) ) echo esc_attr( $ml_stored_meta['director'][0] );?>"/>
        </div>

        <div class="form-group">
            <label for="stars"><?php esc_html_e( 'Stars','ml-domain' );?></label>
            <input type="text" class="full" name="stars" id="stars" value="<?php if( ! empty( $ml_stored_meta['stars'] ) ) echo esc_attr( $ml_stored_meta['stars'][0] );?>"/>
        </div>
        ...
        <div class="form-group">
            <label for="trailor"><?php esc_html_e( 'Youtube ID/ Trailor' );?></label>
            <input class="full" type="text" name="trailor" id="trailor" value="<?php if( ! empty( $ml_stored_meta['trailor'] ) ) echo esc_attr( $ml_stored_meta['trailor'][0] );?>"/>
        </div>
