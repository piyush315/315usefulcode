...
function yvg_add_fields_metabox() {
    add_meta_box( 'yvg_video_fields', __( 'Video Fields' ), 'yvg_video_fields_callback', 'video', 'normal', 'default' );
}

add_action( 'add_meta_boxes_video', 'yvg_add_fields_metabox' );
...
function yvg_video_fields_callback( $post ) {
    ...
    <div class="wrap video-form">
        ...
        <div class="form-group">
            ...
            <input type="text" name="video_id" id="video-id" value="<?php if ( ! empty ($yvg_video_stored_meta['video_id'][0] ) ) echo esc_attr( $yvg_video_stored_meta['video_id'][0] );?>">
        </div>
        ...
        <?php
            if ( isset( $yvg_video_stored_meta['video_id'][0] ) ) :
                ?>
                <iframe width="560" height="315" src="https://www.youtube.com/embed/<?php echo $yvg_video_stored_meta['video_id'][0]; ?>" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>     
                <?php
            endif;
        ?>
    </div>
